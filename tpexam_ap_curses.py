...
def create_user_profile(users,stdscr):
    """Create or retrieve a user profile."""
    user_id = get_input(stdscr, "Enter your user ID: ")
    y, x = stdscr.getyx()
    if user_id in users:
...
def display_timer(time_left, timer_event, question,stdscr):
    ...
    while time_left >= 0 and not timer_event.is_set():
        stdscr.addstr(0, 80, f"Time: {time_left} seconds")
    ...
    if not timer_event.is_set():
        stdscr.addstr(y+1, 0, f"Time's up!")
        
        stdscr.addstr(y+2, 1, f"The correct answer was: {question['correct_answer']}")
        
        stdscr.refresh()
        timer_event.set()

def ask_questions(questions,category,questions_count, time_per_question, user_id, users,stdscr):
        ...
        while True:
            for i, option in enumerate(question['options'], 1):
                if i - 1 == current_option:
                    stdscr.addstr(y+i+1, 0, f"{i}. {option}", curses.A_REVERSE)
                    
                else:
                    stdscr.addstr(y+i+1, 0, f"{i}. {option}")
            ...
            if answer:
                ...
                if answer.isdigit() and 1 <= int(answer) <= len(question['options']):
                    ...
                    if question['options'][choice] == question['correct_answer']:
                        y, x = stdscr.getyx()
                        stdscr.addstr(y, 0, f"Correct answer!")
                        
                        stdscr.refresh()
                        score += 1
                    else:
                        y, x = stdscr.getyx()
                        stdscr.addstr(y, 0, f"Wrong answer. The correct answer was: {question['correct_answer']}")
...
def save_result(user_id, users,questions_count,category, score,stdscr):
...
def export_results_to_csv(user_id, users,stdscr):
    ...
    while True: 
        ...
        if key == curses.KEY_DOWN and current_option < len(menu) - 1:
            ...
            if menu[current_option] == "Exit":
                stdscr.addstr(0, 0, "Exiting Quiz.")
                ...
                users = load_users()  
                user_id = create_user_profile(users,stdscr) 
                while True:
                    ...
                    categories['4'] = 'Exit'
                    stdscr.addstr(0, 0, "Choose a category (-1 to quit Quiz):")
                    ...
                    while True:
                        for idx, (key, value) in enumerate(categories.items()):
                            if idx == category_option:
                                stdscr.addstr(idx + 1, 0, f"{key}. {value}", curses.A_REVERSE)
                        ...
                        if key == curses.KEY_DOWN and category_option < len(categories) - 1:
                            ...
                            if list(categories.values())[category_option] == "Exit":
                                ...
                                stdscr.addstr(0, 0, f"You chose the category: {category}")
                                
                                stdscr.refresh()
                                questions = questions_by_category[category]
                                questions_count = len(questions)
                                result = ask_questions(questions, category, questions_count, TIME_PER_SECOND, user_id, users,stdscr)
                                if not result[1]:
                                    save_result(user_id, users, questions_count, category, result[0],stdscr)
                                stdscr.clear()
                                
                                stdscr.addstr(0, 0, f"Would you like to export your results to a CSV file? (y/n):")
                                
                                stdscr.refresh()
                                option = 0
                                optionYN = {str(i): option for i, option in enumerate({"Yes": "Yes option", "No": "No option"}.keys())}
                                while True:
                                    for idx, (key, value) in enumerate(optionYN.items()):
                                        if idx == option:
                                    ...
                                    if key == curses.KEY_DOWN and category_option < len(categories) - 1:
                                        ...
                                        if optionYN.get(str(option)) == "Yes":
                                            export_results_to_csv(user_id, users,stdscr)
...
#         for idx, item in enumerate(menu):
#             if idx == current_option:
#                 stdscr.addstr(idx, 0, item, curses.A_REVERSE)  # Highlight selected option
#             else:
#                 stdscr.addstr(idx, 0, item)
...
#                 break
#             stdscr.clear()
#             stdscr.addstr(0, 0, f"You selected: {menu[current_option]}")
#             stdscr.refresh()
#             stdscr.getch()
